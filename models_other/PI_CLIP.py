...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        self.base_learnear = nn.Sequential(
            PSPNet_.cls[2], PSPNet_.cls[3], PSPNet_.cls[4]
        )
        ...
        if args.shot > 1:
            ...
            if self.kshot_trans_dim == 0:
                self.kshot_rw = nn.Conv2d(
                    self.shot, self.shot, kernel_size=1, bias=False
                )
                self.kshot_rw.weight = nn.Parameter(
                    torch.ones_like(self.kshot_rw.weight) / args.shot
                )
            else:
        ...
        if self.dataset == "pascal":
            self.bg_text_features = zeroshot_classifier(
                new_class_names, ["a photo without {}."], self.clip_model
            )
            self.fg_text_features = zeroshot_classifier(
                new_class_names, ["a photo of {}."], self.clip_model
            )
        elif self.dataset == "coco":
            self.bg_text_features = zeroshot_classifier(
                new_class_names_coco, ["a photo without {}."], self.clip_model
            )
            self.fg_text_features = zeroshot_classifier(
                new_class_names_coco, ["a photo of {}."], self.clip_model
            )
    ...
    def forward(
        ...
        _, _, query_feat_2, query_feat_3, query_feat_4, query_feat_5 = (
            self.extract_feats(x)
        )
        supp_feat_0, supp_feat_1, supp_feat_2, supp_feat_3, supp_feat_4, supp_feat_5 = (
            self.extract_feats(s_x, mask)
        )
        ...
        supp_feat_item = rearrange(
            supp_feat_item, "(b n) c h w -> b n c h w", n=self.shot
        )
        ...
        tmp_supp_clip_fts, supp_attn_maps = self.clip_model.encode_image(
            s_x_mask, h, w, extract=True
        )[:]
        tmp_que_clip_fts, que_attn_maps = self.clip_model.encode_image(
            x, h, w, extract=True
        )[:]
        ...
        supp_clip_feat_all = [
            aw.reshape(
                tmp_supp_clip_feat_all[0].shape[0],
                tmp_supp_clip_feat_all[0].shape[1],
        ...
        que_clip_feat_all = [
            aw.reshape(
                tmp_que_clip_feat_all[0].shape[0],
                tmp_que_clip_feat_all[0].shape[1],
        ...
        # get the vvp
        if self.shot == 1:
            similarity2 = get_similarity(
                que_clip_feat_all[10], supp_clip_feat_all[10], s_y
            )
            similarity1 = get_similarity(
                que_clip_feat_all[11], supp_clip_feat_all[11], s_y
            )
        else:
            mask = rearrange(mask, "(b n) c h w -> b n c h w", n=self.shot)
            supp_clip_feat_all = [
                rearrange(ss, "(b n) c h w -> b n c h w", n=self.shot)
                for ss in supp_clip_feat_all
            ]
            clip_similarity_1 = [
                get_similarity(
                    que_clip_feat_all[11],
                    supp_clip_feat_all[11][:, i, ...],
                    mask=mask[:, i, ...],
                )
            ...
            clip_similarity_2 = [
                get_similarity(
                    que_clip_feat_all[10],
                    supp_clip_feat_all[10][:, i, ...],
                    mask=mask[:, i, ...],
                )
                for i in range(self.shot)
            ]
        ...
        clip_similarity = F.interpolate(
            clip_similarity,
            size=(supp_feat_cnn.shape[2], supp_feat_cnn.shape[3]),
            mode="bilinear",
            align_corners=True,
        )
        ...
        img_cam_list = [
            F.interpolate(
                t_img_cam.unsqueeze(0).unsqueeze(0),
                size=(supp_feat_cnn.shape[2], supp_feat_cnn.shape[3]),
                mode="bilinear",
                align_corners=True,
            )
            for t_img_cam in img_cam_list
        ]
        img_cam = torch.cat(img_cam_list, 0)
        img_cam = img_cam.repeat(1, 2, 1, 1)

        supp_pro = Weighted_GAP(
            supp_feat_cnn,
            F.interpolate(
                mask,
                size=(supp_feat_cnn.size(2), supp_feat_cnn.size(3)),
                mode="bilinear",
                align_corners=True,
            ),
        )
        supp_feat_bin = supp_pro.repeat(
            1, 1, supp_feat_cnn.shape[-2], supp_feat_cnn.shape[-1]
        )
        ...
        supp_feat_bin = rearrange(
            supp_feat_bin, "(b n) c h w -> b n c h w", n=self.shot
        )
        supp_feat_bin = torch.mean(supp_feat_bin, dim=1)
        query_feat = self.query_merge(
            torch.cat(
                [query_feat_cnn, supp_feat_bin, img_cam * 10, clip_similarity * 10],
                dim=1,
            )
        )

        meta_out, weights = self.transformer(
            query_feat, supp_feat, mask, img_cam, clip_similarity
        )
        ...
        base_map = F.interpolate(
            base_map, size=(map_h, map_w), mode="bilinear", align_corners=True
        )
        ...
        # Output Part
        meta_out = F.interpolate(
            meta_out, size=(h, w), mode="bilinear", align_corners=True
        )
    ...
    def disstil_loss(self, t, s):
        if t.shape[-2:] != s.shape[-2:]:
            t = F.interpolate(
                t.unsqueeze(1), size=s.shape[-2:], mode="bilinear"
            ).squeeze(1)
