...
def main(_run, _config, _log):
    ...
    for label_val, label_name in tqdm(labels.items(), desc="处理标签"):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                # for i, sample in enumerate(tqdm(test_loader, desc="处理样本")):
                ...
                for sub_chunck in range(_config["n_part"]):  # n_part = 3
                    ...
                    query_pred_s = torch.cat(
                        query_pred_s, dim=0
                    )  # [9, 2, 256, 256]  [9, 2, 256, 256]
                    query_pred_s = query_pred_s.argmax(
                        dim=1
                    ).cpu()  # C x H x W   [9, 256, 256]
