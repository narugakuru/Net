...
def cfg():
    ...
    # Network
    # reload_model_path = "./runs/GMRD_CMR_cv0/train/2/snapshots/1000.pth"
    ...
    # exp_str = "_".join([dataset] + [f"cv{eval_fold}"] + [mode])
    exp_str = "_".join([dataset] + [f"cv{eval_fold}"])
...
def add_observer(config, command_name, logger):
    ...
    # exp_str = config["exp_str"]
    exp_str = f'{config["dataset"]}_CV{config["eval_fold"]}_{config["mode"]}'
    exp_name = f"{ex.path}_{exp_str}"
    observer = FileStorageObserver.create(
        # os.path.join(config["path"]["log_dir"], exp_name, config["mode"])
