...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, pretrained_weights="deeplabv3"):
        ...
        # self.criterion_focal = nn.NLLLoss(weight=self.my_weight)
        # self.criterion_focal = nn.CrossEntropyLoss(weight=self.my_weight)
        self.criterion_focal = nn.BCEWithLogitsLoss(pos_weight=self.my_weight[1:])
    ...
    def forward(self, supp_imgs, supp_mask, qry_imgs, train=False):
        ...
        self.thresh_pred = [
            tao[: self.n_ways * self.n_shots * supp_bs],  # support
            tao[self.n_ways * self.n_shots * supp_bs :],  # query
        ]
        ...
        for epi in range(supp_bs):
            ...
            supp_fts_ = [
                [
                    self.getFeatures(
                        supp_fts[0][[epi], way, shot], supp_mask[[epi], way, shot]
                    )
                    for shot in range(self.n_shots)
                ]
                for way in range(self.n_ways)
            ]  # every fig prototype
            fg_prototypes = self.getPrototype(
                supp_fts_
            )  # the prototypes for support images

            ###### Get query predictions ######
            qry_preds = []
            sup_preds = []
            for way in range(self.n_ways):
                qry_pred, sup_pred = self.CART(
                    supp_fts[0][[epi], way, 0],
                    qry_fts[0][epi],
                    fg_prototypes[way],
                    self.k,
                    self.thresh_pred[1],  # query
                )
            ...
            qry_preds = F.interpolate(
                qry_preds, size=img_size, mode="bilinear", align_corners=True
            )
            sup_preds = F.interpolate(
                sup_preds, size=img_size, mode="bilinear", align_corners=True
            )
            ...
            if train:
                align_loss_epi = self.alignLoss(
                    [supp_fts[0][epi]], [qry_fts[0][epi]], qry_preds, supp_mask[epi]
                )
    ...
    def alignLoss(self, supp_fts, qry_fts, pred, fore_mask):
        ...
        pred_mask = torch.where(pred_mask >= ((self.k + 1) / 4), 1, 0).unsqueeze(
            0
        )  # 1 x h x w
        ...
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                fg_prototypes = [
                    self.getPrototype([qry_fts_[n]]) for n in range(len(supp_fts))
                ]
                ...
                preds = torch.sum(torch.stack(preds, dim=0), dim=0) / torch.sum(
                    self.alpha
                )
    ...
    def sup_loss(self, sup_preds, fore_mask):
        ...
        for i in range(len(sup_preds)):
            # Compute sup_loss
            loss += self.criterion_focal(
                sup_preds[i].unsqueeze(0).permute(0, 2, 3, 1),
                supp_label[None, ...].long(),
            )
