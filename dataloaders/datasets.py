...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        if args["dataset"] == "CMR":
            ...
            self.label_dirs = glob.glob(
                os.path.join(args["data_dir"], "chaos_MR_T2_normalized/label*")
            )
        elif args["dataset"] == "SABS":
            self.image_dirs = glob.glob(
                os.path.join(args["data_dir"], "sabs_CT_normalized/image*")
        ...
        if args["dataset"] == "SABS":
            image_pattern = os.path.join(args["data_dir"], "sabs_CT_normalized/image*")
            label_pattern = os.path.join(args["data_dir"], "sabs_CT_normalized/label*")
            print(f"查找图像路径: {image_pattern}")
            self.image_dirs = glob.glob(image_pattern)
            self.label_dirs = glob.glob(label_pattern)

            # 打印找到的文件数量
            print(f"找到的图像文件数量: {len(self.image_dirs)}")
            print(f"找到的标签文件数量: {len(self.label_dirs)}")
        ...
        if len(self.image_dirs) > 0:
            print("\n=== 排序后的前3个文件路径 ===")
            print("图像文件:")
            for path in self.image_dirs[:3]:
                print(f"- {path}")

        # 在移除测试fold后打印信息
        print("\n=== Fold信息 ===")
        print(f"移除前文件数量: {len(self.image_dirs)}")
        # ... 现有的fold移除代码 ...
        ...
        print(f"移除后文件数量: {len(self.image_dirs)}")
        ...
        if self.read:
            ...
            for idx, (image_dir, label_dir, sprvxl_dir) in enumerate(
                zip(self.image_dirs, self.label_dirs, self.sprvxl_dirs), 1
            ):
                print(f"正在读取 {idx}/{total_files}: {os.path.basename(image_dir)}")
                try:
